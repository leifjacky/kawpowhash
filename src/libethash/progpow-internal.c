...
#define PROGPOW_CNT_DAG                 ETHASH_ACCESSES //64
...
hash32_t keccak_f800_progpow2(hash32_t header, uint64_t seed, hash32_t digest)
{
	uint32_t st[25];
	...
	for (int i = 8; i < 16; i++)
		st[i] = digest.uint32s[i-8];

	for (int i=16;i<25;i++)
		st[i] = ravencoin_kawpow[i-16];
		...
		const uint32_t dag_words)
{
	...
	uint32_t offset_g = mix[loop%PROGPOW_LANES][0] % (64 * dag_words / (PROGPOW_LANES*PROGPOW_DAG_LOADS)); // /(16*4)
		...
		)
{
	...
	uint64_t seed = (uint64_t)seed_256.uint32s[1] << 32 | seed_256.uint32s[0];
	...
	digest = keccak_f800_progpow2(seed_256, seed, digest);
